...
| Area                                                | MongoDB (before)                                                                                             | Relational database (after)                                                                                                                                                                                                               |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Two annotators editing the same gene simultaneously | Risk of one user's changes being silently overwritten                                                        | No conflict — each user edits a different row                                                                                                                                                                                             |
...
| Serverless deployment (AWS Lambda, etc.)            | Not feasible — MongoDB requires persistent connections and a running server                                  | SQLite can be bundled with the application or stored on EFS. PostgreSQL works with serverless-friendly managed options like Aurora Serverless or Neon                                                                                     |
| Cold start time                                     | Must establish connection pool to MongoDB on every container or Lambda start                                 | SQLite: opens a local file, near-instant. PostgreSQL: single connection, fast with connection pooling services                                                                                                                            |
...
| Per-gene edit history                               | Grails audit logging tracked changes per feature; users could review a gene's full edit timeline             | Currently a global change stream (assembly-level only). Proposed: add indexed `gene_id` column to change table for instant per-gene history lookups. See [history tracking plan](./apollo2-migration-and-history-tracking.md)             |
...
| One check-result query                   | Loads entire table into memory and filters in application code                                | Push the filter into the database query — standard fix                                                                                          |
...
1. **Add `root_id` column to the feature table** — enables single-query gene
   tree loading, range queries, and deletion; eliminates the recursive traversal
   pattern entirely.
2. **Add `gene_id` column to the change table** — enables per-gene history
   queries with a simple indexed lookup; replaces the unindexable `changedIds`
   JSON array. This is the foundation for the per-gene history viewer and for
   Apollo 2 history migration. See
   [history tracking plan](./apollo2-migration-and-history-tracking.md).
3. **Add `ON DELETE CASCADE` to all foreign keys** — the database handles child
   cleanup automatically on deletion, removing application-level ordering code.
4. **Add missing indexes** (`FeatureEntity.parent`, `RefSeqEntity.assembly`) —
   directly speeds up the most common tree and assembly lookup operations.
5. **Replace loop-based queries with batched `IN` filters** — eliminates the N+1
   query patterns in search, export, and feature count operations.
6. **Replace `LIKE`-based text search with native full-text search** — SQLite
   FTS5 or PostgreSQL tsvector, depending on the deployment target.
7. **Push check result filtering into the database** — one query currently loads
   the entire check results table and filters in memory; this should be a
   database-side filter instead.
8. **Transition from auto-schema to explicit migrations** — generate an initial
   baseline migration, commit migration files to the repository, and run
   migrations on deployment for auditable, reversible schema evolution. See
   [Technical Details](./mikro-orm-technical-details.md#schema-migrations-a-major-improvement-over-mongodb).
9. **Build per-gene history viewer** — right-click a gene to see its full edit
   timeline. Depends on items 1 and 2 above. See
   [implementation plan](./apollo2-migration-and-history-tracking.md#implementation-plan-for-per-gene-history).
10. **Apollo 2 history migration script** — import edit history from an Apollo 2
    PostgreSQL database into Apollo 3's change log, preserving the audit trail
    for users transitioning between systems. See
    [migration plan](./apollo2-migration-and-history-tracking.md#migrating-apollo-2-data-and-history-to-apollo-3).

**Foundation for future improvements:** The flat relational model opens the door
to coordinate range queries using database indexes, server-side parent boundary
validation, per-gene audit logging, Apollo 2 data migration, and other features
that would have been difficult or impossible to add cleanly on the MongoDB
model.

For detailed technical analysis of tradeoffs, schema recommendations, and
